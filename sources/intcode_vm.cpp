...
bool run_intcode_program(vector<int>& o) {
    ...
    queue<int> void_outputs;
    return run_intcode_program(o, empty_inputs, void_outputs);
}

bool run_intcode_program(vector<int>& o, queue<int>& inputs) {
    queue<int> void_outputs;
    return run_intcode_program(o, inputs, void_outputs);
}
...
bool IntcodeProgram::run() {
    //cout << "Running, I/O/P : " << inputs.size() << "/" << outputs.size() << "/" << p << endl;
    long int op;
    long int mode_p0, mode_p1, mode_p2;
    long int p0, p1, p2;
    bool run = true;
    if (halted) {
